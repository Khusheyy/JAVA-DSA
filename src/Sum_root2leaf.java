...
public class Sum_root2leaf {
   ...
   public int sumNumbers(TREES.Sum_root2leaf.TreeNode root) {
      return this.helper(root, 0);
   }

   private int helper(TREES.Sum_root2leaf.TreeNode root, int sum) {
      if (root == null) {
         return 0;
      } else {
         sum = sum * 10 + root.val;
   ...
   public int maxPathSum(TREES.Sum_root2leaf.TreeNode root) {
      this.helper1(root);
      return this.ans;
   }

   private int helper1(TREES.Sum_root2leaf.TreeNode node) {
      if (node == null) {
         return 0;
      } else {
